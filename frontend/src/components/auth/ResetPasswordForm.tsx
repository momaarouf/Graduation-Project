...
export default function ResetPasswordForm({ token: propToken }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-xl overflow-hidden min-h-[480px] flex flex-col">
        ...
        {step === 'code' ? (
          ...
          >
            <div className="text-center space-y-2">
              ...
              {emailFromUrl && (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Check your inbox at <span className="font-medium text-gray-700 dark:text-gray-300">{emailFromUrl}</span>
                </p>
              )}
            </div>
            ...
            <div className="text-center space-y-3 pt-2">
              <Link href="/auth/forgot-password" onClick={() => setDigits(Array(8).fill(''))} className="text-sm text-blue-600 dark:text-blue-400 hover:underline font-medium">
                Resend code
              </Link>
              <p className="text-sm text-gray-500 dark:text-gray-400">
          ...
          >
            <div className="space-y-1">
              {!propToken && (
                <button onClick={handlePrevStep} className="flex items-center gap-1 text-sm text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 mb-4 transition group">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">Choose a strong, unique password.</p>
            </div>

            {/* Note about verification */}
            <div className="px-3 py-2 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-lg">
              <p className="text-[11px] text-blue-600 dark:text-blue-400 flex items-center gap-1.5 leading-tight">
              ...
              {password && (
                <div className="space-y-1.5 pt-1">
                  <div className="flex items-center gap-2">
                    ...
                    {strength.label && <span className="text-[10px] uppercase tracking-wider font-bold text-gray-500 w-20 text-right">{strength.label}</span>}
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              {confirm && passwordsMatch && touched.confirm && (
                <p className="text-[11px] text-emerald-600 flex items-center gap-1 font-medium"><CheckCircle className="w-3 h-3" /> Passwords match</p>
              )}
