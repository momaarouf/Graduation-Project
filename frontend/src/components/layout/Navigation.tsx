...
export default function Navigation() {
 ...
 <span className="text-lg sm:text-xl font-bold text-theme-primary hidden xs:block">
 SafariHub
 </span>
 </Link>
 ...
 className="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:surface-section transition-colors"
 >
 ...
 <span className="text-sm font-medium text-theme-secondary hidden lg:block max-w-[120px] xl:max-w-[180px] truncate">
 {user.email}
 </span>
 ...
 {isUserMenuOpen && (
 <div className="absolute right-0 top-full mt-1 w-64 surface-card border border-theme rounded-xl shadow-xl py-2 z-50">
 ...
 <button
 onClick={toggleTheme}
 className="p-1.5 rounded-lg hover:surface-section"
 >
 {!mounted ? (
 ...
 {isMenuOpen && (
 <div className="md:hidden border-t border-theme pb-3">
 ...
 key={link.href}
 href={link.href}
 className="block px-3 py-2 rounded-lg text-base font-medium text-theme-secondary hover:text-primary-light dark:hover:text-primary-dark hover:surface-section transition-colors"
 ...
 {user && (
 <>
 <div className="px-3 py-2 border-t border-theme mt-2 pt-2 flex items-center justify-between">
 <div className="flex items-center gap-3 min-w-0 flex-1 mr-4">
 <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-base font-bold flex-shrink-0 relative">
 {unreadCounts.total > 0 && (
 <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 bg-red-500 border-2 border-white dark:border-slate-900 rounded-full z-50 shadow-sm" />
 )}
 <div className="w-full h-full rounded-full overflow-hidden flex items-center justify-center">
 {user.avatarUrl && !imageError ? (
 <img 
 ...
 <Link
 href={`/dashboard/${user.role.toLowerCase()}`}
 className="block px-3 py-2 rounded-lg text-base font-medium text-primary-light dark:text-primary-dark hover:bg-primary-light/10 dark:hover:bg-primary-dark/14"
 onClick={() => setIsMenuOpen(false)}
 >
 Go to Dashboard
 </Link>
 ...
 <Link
 href="/auth/login"
 className="block px-3 py-2 rounded-lg text-base font-medium text-theme-secondary hover:text-primary-light dark:hover:text-primary-dark hover:surface-section"
 onClick={() => setIsMenuOpen(false)}
 >
 Sign In
 </Link>
 <Link
 href="/auth/signup"
 className="btn-primary block px-3 py-2 text-base text-center w-full"
 onClick={() => setIsMenuOpen(false)}
 >
 Get Started
 </Link>
