    ...
    <section className="relative min-h-[90vh] flex flex-col items-center justify-center overflow-hidden pt-20">
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 flex flex-col lg:flex-row items-center gap-12">
        ...
        >
          ...
          <p className="text-lg sm:text-xl text-gray-500 dark:text-white/50 font-medium tracking-wide mb-10 max-w-2xl">
          ...
          <motion.div 
            ...
            className="w-full max-w-xl bg-white/95 dark:bg-gray-900/90 backdrop-blur-2xl rounded-[2.5rem] p-3 shadow-2xl border border-white/20 dark:border-gray-800"
          >
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1 relative group bg-gray-50/50 dark:bg-white/5 rounded-2xl transition-all">
                <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5 group-hover:text-orange-500 transition-colors" />
                <input 
                  type="text"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Where to?"
                  className="w-full bg-transparent text-gray-900 dark:text-white border-0 focus:ring-0 font-bold placeholder-gray-400 pl-14 py-4"
                />
              </div>
              <button 
                type="submit" 
        ...
        >
          <div className="relative w-full h-full">
            ...
            <div className="absolute inset-0 bg-blue-500/10 dark:bg-blue-400/5 blur-[100px] rounded-full" />
      ...
      <motion.div 
        ...
        className="absolute bottom-10 left-1/2 -translate-x-1/2 text-gray-400/50"
      >
...
function PremiumShowcase() {
  ...
  return (
    <section className="py-24 px-4 max-w-7xl mx-auto">
      <h2 className="text-4xl sm:text-6xl font-black text-gray-900 dark:text-white mb-16 tracking-tight">Local's Choice.</h2>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        {tours.map((t) => (
          <div key={t.id} className="relative h-[500px] rounded-[3rem] overflow-hidden border dark:border-white/10 shadow-xl">
...
function ProfessionalJourney() {
  ...
  return (
    <section className="py-24 px-4 bg-gray-50/50 dark:bg-transparent transition-colors">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
        {steps.map((s, i) => (
          <div key={i} className="flex flex-col items-center text-center p-8 bg-white dark:bg-white/5 rounded-[2.5rem] border dark:border-white/10 shadow-sm">
            <div className="mb-6 p-4 bg-blue-50 dark:bg-white/5 rounded-2xl text-blue-600 dark:text-orange-400">
              <s.icon className="w-8 h-8" />
            </div>
            <h3 className="text-xl font-black text-gray-900 dark:text-white mb-4">{s.title}</h3>
            <p className="text-gray-500 dark:text-white/40 leading-relaxed">{s.desc}</p>
          </div>
        ))}
      </div>
    </section>
    ...
    <CinematicBackground intensity="high">
      <div className="flex flex-col w-full relative overflow-hidden transition-colors duration-500">
        ...
        <div className="text-center py-20 px-4">
             <Link href="/" className="text-blue-600 dark:text-orange-500 font-bold hover:underline">← Back to Original Side by Side</Link>
        </div>
      </div>
    </CinematicBackground>
  )
