...
export default function OnboardingBanner({ profileCompleted, emailVerified, verificationStatus, role, userEmail }: Props) {
 ...
 {step.statusLabel && (
 <span className="px-1.5 py-0.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[8px] font-bold capitalize tracking-tighter rounded-md border border-accent-light dark:border-accent-dark/20">
 {step.statusLabel}
 </span>
 )}
 </div>
 ...
 <Link href={step.href} className="inline-flex items-center gap-1 mt-2 px-3 py-1.5 text-xs font-bold text-amber-600 hover:text-amber-700 transition capitalize tracking-normal">
