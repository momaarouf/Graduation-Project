            ...
            >
                ...
                {unreadCount > 0 && (
                    <motion.div 
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className="absolute top-1 right-1 w-4 h-4 bg-red-500 text-white text-[10px] flex items-center justify-center rounded-full font-bold border-2 border-white dark:border-gray-950"
                    >
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute right-0 mt-2 w-80 md:w-96 bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-100 dark:border-gray-800 overflow-hidden"
                    >
                        {/* Header */}
                            ...
                            <div className="flex gap-2">
                                {permission === 'default' && (
                                    <button 
                                        ...
                                        className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium px-2 py-1 bg-blue-50 dark:bg-blue-900/30 rounded-md"
                                    >
                                        Enable Push
                                    </button>
                                )}
                            ...
                            {notifications.length === 0 ? (
                                ...
                                <ul className="divide-y divide-gray-50 dark:divide-gray-800">
                                    {notifications.map((n) => (
                                        <li 
                                            key={n.id} 
                                            className={`p-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors cursor-pointer flex gap-3 ${!n.isRead ? 'bg-blue-50/30 dark:bg-blue-900/10' : ''}`}
                                            onClick={() => handleNotificationClick(n)}
                                        >
                                            <div className="flex-1 min-w-0 text-left">
                                                <div className="flex justify-between items-start mb-1">
                                                    <p className={`text-sm font-medium truncate ${!n.isRead ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>
                                                        {n.title}
                                                    </p>
