...
function PaymentCountdownPill({ deadlineUtc, onExpired }: { deadlineUtc: string, onExpired?: () => void }) {
 ...
 <span className="text-[10px] font-bold capitalize tracking-normal">Expired</span>
 </div>
 )
 }
...
import TravelerBookingsLoading from './skeleton'
...
export default function TravelerBookingsPage() {
 ...
 if (isLoading && bookings.length === 0) return <TravelerBookingsLoading />
 ...
 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
  ...
  <h1 className="text-2xl sm:text-4xl font-extrabold text-theme-primary tracking-tight leading-tight capitalize">
  My <span className="text-primary-light">Bookings</span>.
  </h1>
  <p className="text-[10px] text-theme-secondary font-black capitalize tracking-normal opacity-70">
  Journey History & Reservation Hub
  </p>
  </div>
 <Link href="/tours" className="w-full sm:w-auto px-8 py-4 bg-primary-light hover:bg-primary-light-hover text-white rounded-2xl font-black capitalize tracking-[0.2em] text-[11px] transition-all shadow-2xl shadow-primary-light/30 flex items-center justify-center gap-3 active:scale-95">
 ...
 <div className="space-y-4">
  ...
  <button 
    ...
    className={`flex-shrink-0 min-w-[80px] px-5 py-2.5 rounded-xl text-[10px] font-black capitalize tracking-normal transition-all flex items-center justify-center text-center ${
      activeFilter === f 
 ...
 <input type="text" placeholder="Search reservations..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-11 pr-4 py-4 surface-card border border-theme rounded-2xl text-[11px] font-black capitalize tracking-normal text-theme-primary focus:ring-2 focus:ring-primary-light transition-all shadow-sm" />
 </div>
 </div>
 ...
 {filteredBookings.length > 0 ? 
  ...
  <div className="flex flex-col items-center justify-center h-full text-theme-muted opacity-30"><MapPin className="w-6 h-6 mb-1" /><span className="text-[9px] font-bold capitalize tracking-tight">No Media</span></div>
  )}
  ...
  <h3 className="text-[14px] sm:text-xl font-extrabold text-theme-primary line-clamp-1 sm:line-clamp-none capitalize tracking-tight leading-tight">{b.tourTitle}</h3>
  ...
  <Link href={`/dashboard/traveler/bookings/${b.id}`} className="px-2.5 py-1.5 sm:px-4 sm:py-2 bg-surface-base hover:bg-surface-hover border border-theme text-theme-primary text-[9px] sm:text-[10px] font-bold capitalize tracking-normal rounded-lg transition-all flex items-center gap-1 active:scale-95">Details</Link>
  {b.qrCode && b.status === BookingStatus.Confirmed && <Link href={`/dashboard/traveler/bookings/${b.id}/ticket`} className="px-2.5 py-1.5 sm:px-4 sm:py-2 bg-[#16a34a] hover:bg-[#15803d] text-white text-[9px] sm:text-[10px] font-bold capitalize tracking-normal rounded-lg transition-all flex items-center gap-1.5 active:scale-95 shadow-sm shadow-green-500/20"><Ticket className="w-3 h-3" /> Ticket</Link>}
  {b.status === BookingStatus.Completed && !reviewedIds.has(b.id) && <Link href={`/bookings/${b.id}/review`} className="px-2.5 py-1.5 sm:px-4 sm:py-2 btn-accent text-[9px] sm:text-[10px] font-bold capitalize tracking-normal rounded-lg transition-all active:scale-95 flex items-center gap-1 shadow-sm shadow-orange-500/20"><Star className="w-3 h-3" /> Review</Link>}
  ...
  <h3 className="text-xl font-extrabold text-theme-primary capitalize tracking-tight mb-2">History Clear</h3>
  <p className="text-[10px] font-black capitalize tracking-normal text-theme-muted opacity-70 max-w-xs mb-8">You haven't embarked on any expeditions yet. Explore our curated tours to start your story.</p>
  <Link href="/tours" className="btn-primary px-10 py-4 text-[11px] font-extrabold capitalize tracking-[0.2em] rounded-2xl shadow-2xl hover:scale-105 active:scale-95 transition-all">Start Your Search</Link>
  </motion.div>
  )}
 </AnimatePresence>
 </div>
