export default function TravelerBookingsLoading() {
  return (
    <div className="animate-pulse space-y-5">
      ...
      <div className="flex gap-2 border-b border-theme pb-1">
        {[1,2,3,4].map(i => (
          <div key={i} className="h-9 w-24 surface-section rounded-t-lg" />
        ))}
      </div>
      {/* Booking rows */}
      <div className="space-y-3">
        {[1,2,3,4,5].map(i => (
          <div key={i} className="surface-card border border-theme rounded-xl p-4 flex items-center gap-4">
            ...
            <div className="flex-shrink-0 space-y-2">
              ...
              <div className="h-8 w-20 surface-section rounded-lg" />
            </div>
          </div>
        ))}
      </div>
