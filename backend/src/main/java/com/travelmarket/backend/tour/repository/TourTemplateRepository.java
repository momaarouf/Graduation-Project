...
public interface TourTemplateRepository extends JpaRepository<TourTemplate, Long> {
    ...
    @Query("""
        ...
        WHERE t.status = :status
          ...
          AND (:query IS NULL OR (LOWER(t.title) LIKE :query OR LOWER(t.description) LIKE :query OR LOWER(u.fullName) LIKE :query OR LOWER(t.city) LIKE :query OR LOWER(t.locationName) LIKE :query OR LOWER(t.region) LIKE :query OR LOWER(t.category) LIKE :query OR LOWER(t.tags) LIKE :query))
